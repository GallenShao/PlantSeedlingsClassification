...
def build_net():
	...
	pool1 = pool_layer(conv11, 3, 2)
	
	conv2 = conv_layer(pool1, [3, 3, 64, 128], 1)
	conv22 = conv_layer(conv2, [3,3,128,128],1)
	pool2 = pool_layer(conv22, 3, 2)
	
	conv3 = conv_layer(pool2, [3, 3, 128, 256], 1)
	conv4 = conv_layer(conv3, [3, 3, 256 ,256], 1)
	conv5 = conv_layer(conv4, [3, 3, 256, 256], 1)
	pool5 = pool_layer(conv5, 3, 2)
	
	flat = tf.reshape(pool5, [-1, 7*7*256])
	fc1 = dense_layer(flat, [7*7*256, 4096], tf.nn.relu, keep_prob)
	fc2 = dense_layer(fc1, [4096, 128], tf.nn.relu, keep_prob)
	output_y = dense_layer(fc2, [128, 12], tf.nn.softmax, keep_prob)
...
def main():
	Xtrain, Ytrain, XCV, YCV, Xtest, Ytest, categories = data.load_data(shuffle=False, size = 51)
